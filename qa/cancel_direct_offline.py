...
class CancelDirectOfflineTest(OpenBazaarTestFramework):
    ...
    def run_test(self):
        ...
        r = requests.get(api_url)
        if r.status_code != 200:
            raise TestFailure("CancelDirectOfflineTest - FAIL: Could not get listing index")
        resp = json.loads(r.text)
        ...
        r = requests.get(api_url)
        if r.status_code != 200:
            raise TestFailure("CancelDirectOfflineTest - FAIL: Could not load order from Bob")
        resp = json.loads(r.text)
        ...
        r = requests.get(api_url)
        if r.status_code != 200:
            raise TestFailure("CancelDirectOfflineTest - FAIL: Could not load order from Alice")
        resp = json.loads(r.text)
        if resp["state"] != "CANCELED":
