...
class PurchaseCryptoListingTest(OpenBazaarTestFramework):
    ...
    def run_test(self):
        ...
        if resp["state"] != "AWAITING_PAYMENT":
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Bob purchase saved in incorrect state")
        if resp["funded"] == True:
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Bob incorrectly saved as funded")
        if resp["contract"]["vendorListings"][0]["metadata"]["coinType"] != "ETH":
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Bob incorrectly saved without a coinType")
        if resp["contract"]["buyerOrder"]["items"][0]["paymentAddress"] != "crypto_payment_address":
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Bob incorrectly saved without a paymentAddress")

        # check the sale saved correctly
        api_url = alice["gateway_url"] + "ob/order/" + orderId
        r = requests.get(api_url)
        if r.status_code != 200:
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Couldn't load order from Alice")
        resp = json.loads(r.text)
        if resp["state"] != "AWAITING_PAYMENT":
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Alice purchase saved in incorrect state")
        if resp["funded"] == True:
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Alice incorrectly saved as funded")
        if resp["contract"]["vendorListings"][0]["metadata"]["coinType"] != "ETH":
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Alice incorrectly saved without a coinType")
        if resp["contract"]["buyerOrder"]["items"][0]["paymentAddress"] != "crypto_payment_address":
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Alice incorrectly saved without a paymentAddress")
        ...
        if resp["state"] != "AWAITING_FULFILLMENT":
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Bob failed to detect his payment")
        if resp["funded"] == False:
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Bob incorrectly saved as unfunded")
        if resp["contract"]["vendorListings"][0]["metadata"]["coinType"] != "ETH":
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Bob incorrectly saved without a coinType")
        if resp["contract"]["buyerOrder"]["items"][0]["paymentAddress"] != "crypto_payment_address":
            raise TestFailure("PurchaseCryptoListingTest - FAIL: Bob incorrectly saved without a paymentAddress")
