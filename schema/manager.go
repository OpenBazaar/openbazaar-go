...
func (m *openbazaarSchemaManager) InitializeIPFSRepo() error {
	...
	if err := m.addConfigExtensions(); err != nil {
		return err
	}
...
func (m *openbazaarSchemaManager) addConfigExtensions() error {
	var (
		w = WalletConfig{
			Type:             "spvwallet",
			MaxFee:           2000,
	...
	if err := r.SetConfigKey("Bootstrap-testnet", TestnetBootstrapAddresses); err != nil {
		return err
	}
