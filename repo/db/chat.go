...
func (c *ChatDB) MarkAsRead(peerID string, subject string, outgoing bool, messageId string) (string, bool, error) {
	...
	stmt2, err := c.db.Prepare("select messageID from chat where subject=?" + peerStm + " and outgoing=?")
	if err != nil {
		return "", updated, err
	}
	defer stmt2.Close()
	...
	if peerID != "" {
		rows, err = stmt2.Query(subject, peerID, outgoingInt)
	} else {
		rows, err = stmt2.Query(subject, outgoingInt)
	}
	if err != nil {
		return "", updated, err
	}
