...
func TestMigration024(t *testing.T) {
	...
	examples := map[string]migrations.Migration024_TransactionRecord_beforeMigration{
	...
	for _, tx := range examples {
		transactions, err := json.Marshal([]migrations.Migration024_TransactionRecord_beforeMigration{tx})
		if err != nil {
			t.Fatal(err)
		}
	...
	if err = ioutil.WriteFile(repoVerPath, []byte("23"), os.ModePerm); err != nil {
		t.Fatal(err)
	}

	// Test migration up
	if err := (migrations.Migration024{}).Up(appSchema.DataPath(), dbPassword, true); err != nil {
		t.Fatal(err)
	}
	...
	for purchaseRows.Next() {
		var (
			...
			actualTransactions    []migrations.Migration024_TransactionRecord_afterMigration
		)
		...
		Migration024_assertTransaction(t, examples, actualTransactions)
	}
	...
	// Test migration down
	if err := (migrations.Migration024{}).Down(appSchema.DataPath(), dbPassword, true); err != nil {
		t.Fatal(err)
	}
	...
	for purchaseRows.Next() {
		var (
			marshaledTransactions sql.NullString
			orderID, paymentCoin  string
			actualTransactions    []migrations.Migration024_TransactionRecord_beforeMigration
		)
		if err := purchaseRows.Scan(&orderID, &marshaledTransactions, &paymentCoin); err != nil {
			t.Error(err)
			continue
...
func Migration024_assertTransaction(t *testing.T, examples map[string]migrations.Migration024_TransactionRecord_beforeMigration, actual []migrations.Migration024_TransactionRecord_afterMigration) {
